...
public final class OAuth2AccessTokenJsonDeserializer extends StdDeserializer<OAuth2AccessToken> {
	...
	public OAuth2AccessToken deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException,
		...
		while (jp.nextToken() != JsonToken.END_OBJECT) {
			...
			jp.nextToken();
			if (JsonOAuth2AccessTokenConstants.ACCESS_TOKEN.equals(name)) {
				tokenValue = jp.getText();
			}
			else if (JsonOAuth2AccessTokenConstants.TOKEN_TYPE.equals(name)) {
				tokenType = jp.getText();
			}
			else if (JsonOAuth2AccessTokenConstants.REFRESH_TOKEN.equals(name)) {
				refreshToken = jp.getText();
			}
			else if (JsonOAuth2AccessTokenConstants.EXPIRES_IN.equals(name)) {
				expiresIn = jp.getLongValue();
			}
			else if (JsonOAuth2AccessTokenConstants.SCOPE.equals(name)) {
